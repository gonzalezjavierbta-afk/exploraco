...
| admin.js | GET/POST | Recursos admin: solicitudes, rese\u00f1as, destacado, notificaciones; v2 (TSK-103 / ADR-028): `categoria` en `consumibles_lista`/`consumibles_crear`/`consumibles_editar` (400 `CATEGORIA_INVALIDA`); v3/v4 (ADR-053 / TSK-148, 2026-09-21): rama NUEVA **`GET ?recurso=salud_red`** (router real `?recurso=`, gate Bearer `auth()`) con por_dia/por_accion/caps/`distribucion_nivel {derivado, visible}`/exentos/`nivel_max_vs_derivado`/alertas desde `xp_ledger`, degradacion 42P01, y espejo `NIVEL_DERIVADO_SQL` validado por `smoke_niveles_espejos.js`; v5 (ADR-055 / TSK-151, 2026-09-23): rama `?recurso=mercado` (config por Casa + moderacion de ofertas); **v6 (ADR-058 / TSK-155, 2026-09-24)**: `salud_red` agrega 4 bloques ADITIVOS de origen y lejania (`distribucion_origen`, `mult_origen_stats` + top outliers, `config_origen`, `alertas_origen`; degradan 42703 con warn si la 038 no corrio; patron BUG-021) |
...
- **`geo_ciudades` (038):** municipios de Colombia con columnas EXACTAS del seed real (`db/seeds/geo_ciudades_seed.json`): **1.122 filas DIVIPOLA (1.103 Municipio + 18 Area no municipalizada + 1 Isla)**; PK natural `cod_mpio` (idempotencia del loader `ON CONFLICT (cod_mpio) DO NOTHING`), `es_capital` DERIVADA persistida (cod_mpio termina en '001') para desempatar homonimos (`ORDER BY es_capital DESC, cod_mpio ASC`), `pais_iso2 char(2) DEFAULT 'CO'`, indice por nombre normalizado (`idx_geo_ciudades_nc`).
- **`geo_paises` (038):** centroides por ISO-3166-1 alfa-2 (columnas del seed real `db/seeds/geo_paises_seed.json`), **245 filas**; `iso2` PK (coincide con `usuarios.pais_base` en mayusculas).
- **`usuarios.origen_declarado_en` (038):** `timestamptz NULL`; elegibilidad a Nomada/Extranjero (>= 7 dias) y **ANTI-TELEPORT** (lo fija el backend v22 al cambiar `ciudad_base`/`pais_base`); backfill UNICO a la migracion para usuarios que ya declararon ciudad/pais (no sobrescribe no nulos).
- **`xp_ledger.mult_origen` / `origen_tier` (038):** `numeric(10,6) NOT NULL DEFAULT 1` y `text NULL` con CHECK idempotente; el motor persiste el factor de origen y su tier por evento (reporte y auditoria).
- **`gamificacion_config` (038):** 7 claves nuevas de la curva: `factor_origen_local=1.0000`, `factor_origen_nomada_max=1.2000`, `factor_origen_extranjero_max=1.4000`, `origen_km_local=25`, `origen_km_nomada=1000`, `origen_km_extranjero=3000`, `origen_min_dias_cuenta=7` (seed `ON CONFLICT DO NOTHING`; parametriza la curva sin deploy).
- **Endpoint:** `salud_red` en `api/admin.js` v6 con 4 bloques aditivos de origen (ver seccion 2); `api/usuarios.js` v22 expone el objeto `origen`. Cero endpoints nuevos.
- **Paso manual obligatorio (BLOQUEANTE):** la **038 + el seed geo deben estar aplicados en Neon ANTES del deploy del backend v29/v30** (patron BUG-021/BUG-060; sin las columnas el ledger reintenta y degrada, y `salud_red` degrada 42703). **YA aplicados (2026-09-24).** La 038 NO incluye el seed (se carga por separado con `scripts/seed_geo.js`).
- **Rollback (LOSSY si ya hay ledger con origen):** `DROP TABLE geo_ciudades/geo_paises`; `ALTER TABLE usuarios DROP COLUMN origen_declarado_en`; `ALTER TABLE xp_ledger DROP COLUMN mult_origen, origen_tier`; borrar las 7 claves de config. Respaldar `xp_ledger` antes (ADR-003: no va en flujo normal).
- **Detalle funcional:** curva escalonada JS/SQL (Local x1.00; Nomada top 1.20 @1000 km; Extranjero top 1.40 @3000 km; sin tier/punto = 1.00); `mult_origen` HERMANO de `stack_temp` dentro de `cap_global`; espejos `normGeo`/`sqlNormGeo` y `calcularFactorOrigen`/`sqlCurvaFactorOrigen` validados por `smoke_origen_factor_parity.js` (111/111, gate `npm run smoke:origen`). Contrato vigente en DECISIONS.md ADR-058; cierre en TASKS.md TSK-155.
