...
## Completado reciente
- Feature "Mis mapas personales (comunidad) con paridad al mapa cultural del index" / ADR-045 -- TSK-133 (2026-09-19, working tree, **SIN commitear**) - NUEVOS assets frontend `mapa-cultural.js` (motor `window.MapaCultural`, ASCII-safe, 65282 bytes, L1597) y `mapa-cultural.css` (121 reglas scopadas bajo `.mc-root`, 0 `!important`) + `scripts/smoke_mapa_cultural.js` (56/56 PASS); MODIFICADOS `mymapa.js` (+154/-46; consume `MapaCultural.create` L125, elimina Leaflet propio y `bindPopup`) y `comunidad.html` (+17/-0; `<link>` L14 y `<script>` L559 antes de `mymapa.js` L561). `index.html`/`api/*` intactos (diff vacio). Presupuesto **8/8 INTACTO**; sin migraciones. QA **APTO CON OBSERVACIONES**. Mitiga (no cierra) **BUG-061** con `jsonAuthHeaders()`. Tarea pendiente **TSK-134** (migracion del index a `mapa-cultural.js`). Detalle en `TASKS.md` TSK-133 y en la sesion de "Que se estaba haciendo".
...
### Motor compartido del Mapa Cultural `mapa-cultural.js` (2026-09-19) - ADR-045 / TSK-133

Feature **IMPLEMENTADA EN WORKING TREE** (SIN commitear; verificado contra archivo real, ADR-006, el 2026-09-19). Origen: pedido directo del usuario de "Mis mapas personales (comunidad) con paridad al mapa cultural del index". La decision vive en `DECISIONS.md` **ADR-045**; la tarea en `TASKS.md` TSK-133. NO hay archivos nuevos en `api/` (8/8 intacto, ADR-001/ADR-010) ni migraciones; SI hay 3 assets/scripts nuevos sin versionar.

**Estado real del working tree (verificado, ADR-006, 2026-09-19):**
- `git status`: `M comunidad.html`, `M mymapa.js`; untracked `?? mapa-cultural.js`, `?? mapa-cultural.css`, `?? scripts/smoke_mapa_cultural.js`.
- `mapa-cultural.js`: 65282 bytes, 0 bytes >127, `window.MapaCultural` L1597, API multi-instancia (`create`/`init` + `setPlaces`/`setMedia`/`setMediaEnabled`/`setMediaTypes`/`refresh`/`getMap`/`openDrawer`/`closeDrawer`/`destroy`) + helpers; `jsonAuthHeaders` L1354.
- `mapa-cultural.css`: 16112 bytes, 121 reglas / 121 llaves, 0 `!important` reales, scope `.mc-root`.
- `scripts/smoke_mapa_cultural.js`: 56/56 PASS (`node scripts/smoke_mapa_cultural.js`).
- `mymapa.js`: +154/-46; `MapaCultural.create` L125; sin `bindPopup` propio.
- `comunidad.html`: +17/-0; `<link>` L14, `<script>` L559 (antes de `mymapa.js` L561).
- `index.html` / `api/*` / `index-api-connector.js`: SIN CAMBIOS (diff vacio).

**Que se hizo (resumen; ver TASKS.md TSK-133 para anclas ADR-006):**
- **Motor compartido:** pines por categoria, clustering por proximidad de 40 px, drawer completo (hero/badge/rating/precio/lead/tabs multimedia/"Ver lugar completo"), capa de media (iconos, bounds, tope 300, dedupe) y lightbox/album; normalizacion unica `normalizePlace`/`normalizeMedia`.
- **Paridad:** tiles CARTO Voyager + clustering 40 px + drawer completo al clic en pin.
- **Capa de media:** solo items de los destinos del mapa activo (match estricto por slug; `origen='album'` excluido), un unico fetch cacheado a `?tipo=multimedia_mapa`, default ON si el mapa activo tiene media.
- **`mymapa.js`:** consume `MapaCultural`, retira su Leaflet propio y `bindPopup`.
- **Mitigacion BUG-061:** `guardarMedia`/`votarMedia` envian `Authorization` via `jsonAuthHeaders()` (backend sigue ABIERTO).

#### Que sigue
1. **Migracion de `index.html` a `mapa-cultural.js`** (TSK-134), en entrega CONTROLADA; riesgos: contrato `window.mapaMap` via `onMapReady`, helpers compartidos que no deben migrar, colisiones `.md-link`/`.md-close`, lightbox y ~1190 lineas de extraccion 1:1.
2. **QA visual en navegador** del tab Mapa de `comunidad.html`: drawer al clic en pin, toggle de media, lightbox/album y regresion con Leaflet.
3. **Validar el shape real de `?tipo=mapa` contra Neon** (el smoke cubre ambos shapes; falta dato real de la tabla).
4. **Deploy** de `mapa-cultural.js`/`mapa-cultural.css` + `mymapa.js`/`comunidad.html` (assets frontend; no requieren migracion ni backend).
5. **Comandos de verificacion:** `node scripts/smoke_mapa_cultural.js` (56/56 PASS) y Escudo GOLD (`node --check mapa-cultural.js`, `node --check mymapa.js`, ASCII 0 bytes >127 en los 3 nuevos, divs `comunidad.html` 319/319, llaves CSS 121/121).
6. **Commit + push** de los 5 archivos (2 modificados + 3 untracked), sin mezclar ajenos.

#### Riesgos activos
- **Doble motor temporal:** `index.html` conserva su mapa inline hasta TSK-134; evitar que divergan.
- **BUG-061 ABIERTO:** la mitigacion con Bearer no corrige el backend de `guardar_media`/`tipo='foto'`; escalado a `sql-security`.
- **QA visual en navegador pendiente:** el smoke es Node vm (mock); validar en real antes de cerrar TSK-133.
