...
- **EN CURSO / SIN COMMITEAR (2026-09-22, sesion express):** (A) **galeria unificada desde la ficha** -- el clic en cualquier foto de un item ya NO abre el lightbox simplificado `#lb` (ELIMINADO de `api/pagina-destino.js`, +29/-68) sino que navega directo a `/galeria.html?destino=<slug>#g-foto=<url>` (helper `irAFotoGaleria` con `GAL_FOTOS`/`HERO_FOTOS`), y `galeria.html` consume el hash con `gAutoAbrirHash()` para abrir el visor completo (voto/guardar/compartir/comentarios) SOBRE esa foto; (B) **recalibracion de `XP_BASES`** en `api/interacciones.js` (guardado 3, voto_media 3, rating 5, chat_comentario 6, visita 30, foto_viajero 30, album_crear 25, album_foto 20); (C) **v27 "Rising Star Decay" del voto de media** (autorizado por el operador, REVIERTE el rechazo de ADR-053): XP decreciente con carga ponderada por tiempo (`factor = 1 - carga/20`; se recarga a full a las 24h) + cooldown creciente (`min(600, carga*30)` s, `VOTO_COOLDOWN_*`); tope duro 20/24h INTACTO; `registrarVotoMedia` responde 429 con `faltan`. Verificacion: `node --check` OK, ASCII 0, backticks 0; guard `scripts/check_buildHTML_inline.js` actualizado (`abrirLightbox` -> `irAFotoGaleria`). Deuda `[DEUDA-EXPRESS]`: **nuevo ADR que revierta ADR-053 (Decay)**; CSS huerfano de `#lb` en `pagina-destino.js`; `directorio-session.js:151` (+5 XP local) y >100 fichas estaticas (+25/+8) desalineadas con la nueva escala; **XP por subir lugar nuevo (`publicar_lugar`) SIN wiring** (endpoint anonimo, sin `usuario_id` en `destinos`); calibracion fina de la curva; commit/deploy pendientes.
...
- Que sigue: **[TSK-156 Pantallas de entrada (2026-09-24)] 1) incluir `scripts/smoke_ref_info.js` (untracked, gate 26/26 PASS) + `package.json` (M) en el commit (el gate YA existe y YA esta encadenado a `npm test` como primer smoke); 2) commit + push de `api/usuarios.js`, `api/interacciones.js`, `registro.html`, `index.html`, `scripts/smoke_ref_info.js` y `package.json` (sin migraciones; Vercel despliega al push; `vercel.json` ya sirve `/(.*)\.js` con `no-store`); 3) QA runtime post-deploy: overlay 1 sola vez (`ec_welcome_visto`), banner con NOMBRE del anfitrion via `ref_info`, reclamo post-alta `reclamar_bonus_referido`, y REFERIDO_INVALIDO con "Continuar sin invitacion".** | **[ADR-006] NOTA DE REALIDAD:** el release TSK-155/ADR-058 (Origen) YA esta commiteado y en `origin/main` (`e5a59f8` + `27784f8`); el texto heredado de este mega-bullet sobre su COMMIT quedo desactualizado, lo vigente de TSK-155 es solo QA runtime post-deploy (re-ejecutar `npm run smoke:origen` 111/111) + FASE 2 diferida. | **COMMIT + DEPLOY del release ADR-058 / TSK-155 (Multiplicador de Origen por lejania):** `git add db/migrations/038_origen_lejania.sql db/seeds/ scripts/seed_geo.js api/interacciones.js api/usuarios.js api/admin.js admin.html index.html mi-perfil.html usuario-session.js scripts/smoke_058_origen_clasificador.js scripts/smoke_origen_factor_parity.js docs/superpowers/specs/2026-09-24-multiplicador-origen-lejania-design.md` + commit + push (Vercel despliega al push). **ORDEN OBLIGATORIO ya cumplido:** migracion **038 + seed geo APLICADOS en Neon el 2026-09-24** ANTES del deploy del backend v29/v30 (patron BUG-021/BUG-060) -- el backend degrada a 1.00 sin la 038 pero el ledger reintenta; NO desplegar el frontend del badge origen sin el backend v22. **QA runtime en produccion post-deploy:** con sesion real, un Local cerca del punto debe ver el multiplicador ~1.00, un Nomada lejano ~1.20 y una cuenta extranjera elegible ~1.40 (ver desglose de XP); verificar que cambiar `ciudad_base` congela el factor 7 dias (anti-teleport); verificar `salud_red` con los 4 bloques de origen; **re-ejecutar `npm run smoke:origen` (111/111) tras el deploy**; **FASE 2 (DIFERIDA):** disenar con `architect` la verificacion documental de nacionalidad (alternativa 2 del ADR-058) y el auto-update del seed DIVIPOLA; **COMMIT + DEPLOY del release TSK-153 / ADR-057 (guardar album + XP dual 5/10 + "Mi Museo" fuera del mapa publico):** `git add api/interacciones.js galeria.html comunidad.html mapa-cultural.js scripts/smoke_gamificacion_v6.js scripts/smoke_038_casas_clases.js` + commit + push (Vercel despliega al push; sin migracion y sin cache-bust, `vercel.json` sirve todo `/(.*)\.js` con `no-store`); **QA runtime en produccion:** con sesion real, abrir el visor de un album AJENO en `galeria.html`/`comunidad.html`, pulsar Guardar (debe acreditar +5 al ejecutor y +10 al dueno SOLO la primera vez), confirmar que el album aparece en "Mis guardados" de `mi-perfil.html` y que el album "Mi Museo" NO pincha en el mapa publico; **FASE 2 (DIFERIDA):** disenar con `architect` la curaduria de MODIFICACION de fichas (proponer/votar/moderar, patron Activos Ocultos) + decidir si se necesita un flag `albumes.personal` (alternativa 1 del ADR-057) para cubrir albumes personales con otro nombre; **QA visual OPCIONAL en navegador de la pagina `taller-de-las-moscas` (TSK-150, ya publicada y verificada por API/sitemap/smoke en produccion el 2026-09-21/22)** + commit de los 4 archivos untracked (`ficha/taller de las moscas.json`, `scripts/seed-taller-de-las-moscas.js`, `scripts/load-taller-de-las-moscas-api.js`, `scripts/smoke_test_taller-de-las-moscas.js`) en el proximo pase de docs; NO tocar los 2 `catch` vacios del seed (patron de los ~106 seeds, deuda `[DEUDA-EXPRESS]`); **QA runtime en produccion del release ADR-054 (TSK-149):** navegador con sesion real para validar guardados en "Mis Albumes", el toggle "Hacer publico" y el fix BUG-082 (la migracion 032 y el release ya estan aplicados/desplegados: `b4ad861` en `origin/main`; sin cache-bust, `vercel.json` sirve todo `/(.*)\.js` con `no-store`); **Gamificacion v6 (TSK-148 / ADR-053) DESPLEGADA: migracion 031 en Neon + backend v25/v19/v4 (`9efbfc7`) + frontend/8 espejos (`d803ce7`) en `origin/main`; Vercel despliega al push y no requiere cache-bust (todo `/(.*)\.js` va con `no-store`). Verificar en produccion: toast con desglose, barra de progreso, UI de cupo y pestana "Salud de la Red"**; **DEPLOY del release v24 (TSK-147: ADR-051 ubicacion por recurso + ADR-052 carpetas de guardados + BUG-081 fix de seguridad `scope=mio`) en orden 029/030 (YA APLICADAS en Neon el 2026-09-21) -> backend `api/interacciones.js` v24 -> frontend (`mi-perfil.html`/`mymapa.js`/`map-picker.js`/`index-api-connector.js` con cache-bust `?v=7`/`?v=4`/`?v=2`/`?v=2`) + correr/documentar los smokes nuevos de 029/030 + fix del reset de `museoQuitarCoords`**; **DEPLOY del fix de media del mapa cultural (BUG-080 / TSK-146: `mapa-cultural.js` + cache-bust `?v=7` en `index.html` y `comunidad.html`) para que llegue a produccion + ampliar `scripts/smoke_mapa_cultural.js` con regresion**; **APLICAR la migracion 028 en Neon ANTES del deploy del backend (BLOQUEANTE, 42703)** y luego commit/deploy del release (admin.html + api/admin-destinos.js + api/destinos.js + 028); luego el resto del 2026-09-20 ya registrado: QA visual en navegador del estado persistente (los 5 directorios + la ficha en una sola sesion) y del drawer (TSK-135 + TSK-143); commit/deploy de TSK-144 + TSK-143 (assets frontend con cache-bust `?v=5` + `api/pagina-destino.js`); deuda legacy R2 de `publicarResena` (consumidores `.then(function(ok){ if(ok) })` fuera de flujo Vercel); deuda de los 103 `load-*-api.js` + `api/publicar-lugar.js` + `scripts/upload-eventos.js` que NO envian `zona` (fichas/eventos por esas vias quedan sin zona); decision de producto sobre `comunidad.html` (video/audio de comunidad no guardado SIN superficie); aprobar/aplicar las propuestas de gobernanza (PENDIENTE DE APROBACION del operador); commitear el doc de analisis ANALISIS_AI-DOS_v1.1_y_REGLAS_DE_ORO_v5.md y el .docx (untracked); deploy del release; confirmar la migracion 023 en Neon; **BUG-079 (CERRADO, fix en working tree):** correr `scripts/diagnose_fotos_huerfanas.js` (NUEVO, read-only) en Neon para medir los votos de fotos curadas YA huerfanos por el REPLACE historico y decidir re-anclaje (con backup); limpiar codigo muerto `_syncFotosGaleria` (`admin.html` L6193, 0 call-sites) + corregir la cita de `GUIA_DE_DESARROLLO.md` L482 (doc-drift); decidir la exclusividad de `es_hero` (hoy gana la ultima si el front manda dos); coordinar el commit/deploy del release BUG-079 (`admin.html` v9.20260920 + `api/admin-destinos.js` v2.2) con el de 028.
- Riesgos activos: **[TSK-156 Pantallas de entrada (2026-09-24)]** antes del commit/deploy: `scripts/smoke_ref_info.js` existe (untracked, 26/26 PASS) y ya esta encadenado a `npm test` pero SIN commitear (incluirlo en el commit del lote); deuda IDOR preexistente en `reclamar_bonus_referido` (no exige `validarSesion`; escalada a `sql-security`); bytes>127 preexistentes en UI visible de `registro.html` y motor viejo de `index.html` (heredados, ASCII 0 solo en codigo NUEVO); z-index del overlay 10000 verificado (bajo bono-ref 10002/expEra 10001, sobre login 9999); **`<link>` a Geist SIN USAR en `registro.html:12`** (carga muerta, deuda de limpieza; la afirmacion previa "DESCARTADO/grep 0" era INCORRECTA, ADR-006). | **[ADR-058 / TSK-155 YA EN origin/main]** (e5a59f8 + 27784f8, 2026-09-24): el estado "EN WORKING TREE" de este mega-bullet quedo resuelto; restan QA runtime post-deploy y el nerf M-4 visible (usuarios sin `ciudad_base` pasan de x1.2 a 1.00) + FASE 2 diferida. | **ADR-058 / TSK-155 EN WORKING TREE (2026-09-24):** migracion **038 + seed geo APLICADOS en Neon** (1.122 ciudades / 245 paises); el backend v29/v30 y el frontend del badge NO estan desplegados (sin ventana de 503; mientras no se despliegue, el factor queda en 1.00). **Nerf M-4 consciente:** los usuarios sin `ciudad_base`/punto pasan de x1.2 (bono ADR-028) a **1.00** al desplegar v29/v30 -- aceptado por el operador, pero es un cambio de comportamiento visible en el desglose de XP. **Origen auto-declarado:** mitigado con `email_verified` + antiguedad (7 dias) + anti-teleport + monitoreo en `alertas_origen`; sin verificacion documental (deuda). **Homonimos de municipios:** desempate por `es_capital DESC, cod_mpio ASC` (geo_ciudades). **BUG-021/BUG-060:** la 038 + seed YA estan en Neon (resuelto); el drift ADR-053 20->40 niveles sigue siendo deuda de arrastre. **ADR-054 DESPLEGADO (TSK-149):** migracion 032 aplicada en Neon el 2026-09-21 (idempotencia verificada) + release en `origin/main` (`b4ad861`); sin ventana de 503 activa. **El DROP es IRREVERSIBLE (consumado):** la 032 descarto la organizacion previa en carpetas (perdida aceptada por el operador; rollback lossy, ADR-054). **BUG-082 (ALTA) DESPLEGADO:** `api/pagina-destino.js` ya filtra `af.visible=true` (L2762) en produccion; queda QA runtime. **Gamificacion v6 DESPLEGADA (TSK-148 / ADR-053):** backend v25/v19/v4 (`9efbfc7`) + frontend/8 espejos (`d803ce7`) ya estan en `origin/main` (push 2026-09-21); sin cache-bust necesario (`vercel.json` -> `no-store` para todo `/(.*)\.js`). Residual: verificar en produccion el toast con desglose, la barra de progreso, la UI de cupo y la pestana "Salud de la Red". **Migraciones 027 y 028 siguen SIN aplicar (arrastre):** la 031 se aplico sin depender de ellas, pero `consumibles.precio_xp_base`/vista `consumibles_precio` (027) y `destinos.zona` (028) siguen sin efecto; si se aplica la 027 DESPUES de la 031, re-sembrar `precio_xp_base` desde `precio_xp` (ADR-053 Decision 12 / R-2). **Deuda abierta de la Gamificacion v6:** `spot_atributos` "nunca al creador" no es enforceable sin `destinos.creado_por` (se implemento dedup por `(usuario,destino)` contra `xp_ledger`); el backend aun NO expone un contrato unico `estado_cupo` (la UI informa solo lo que algunas ramas ya mandan: 429, `tope_diario`); >100 fichas estaticas legacy con parches inline de XP en `localStorage` (+25/+8 XP) que no llaman a la API (deuda de migracion al renderer dinamico); `scripts/smoke_test_epic_prompt.js` con **4 FAIL preexistentes** (DQ-2: vocaciones 3->4 y filtro de `chat_salas`); `gamificacion_config` NO parametriza umbrales (R-6: recalibrar la curva 42000 exige deploy); techo 42000 sin datos de calibracion (7 usuarios, max ~1630 XP). **Fuga BUG-081 sin desplegar (ALTA/CRITICA):** `api/interacciones.js` v24 corrige en working tree la exposicion de media privada de terceros en `multimedia_mapa scope=mio` (antes sin sesion); mientras no se despliegue el backend v24, produccion mantiene la version vulnerable. **ORDEN de deploy del v24 (029/030 YA en Neon):** desplegar backend v24 ANTES del frontend; si el front va primero, `lat_propia`/`carpetas` no existen y el `scope=mio` con Bearer no tendra contraparte; deuda `museoQuitarCoords` sin reset al cerrar modal. **Cache `?v=7` del mapa cultural sin desplegar (BUG-080 / TSK-146):** el fix de render de media vive solo en working tree; sin commit/deploy, produccion mantiene el render congelado y el atajo "Todo" inoperante, y el video reportado "rastro mc-trampas" no existe en la BD (0 filas) pese a que el backend SI devuelve la media. **ORDEN 028 -> deploy del backend (BLOQUEANTE):** si se despliega `api/admin-destinos.js`/`api/destinos.js` sin aplicar la 028 en Neon, `42703 column does not exist` (los SELECT/INSERT ya referencian `d.zona`); tambien deuda de zona en 103 loaders + `publicar-lugar.js` + `upload-eventos.js` (no envian `zona`), zona NO visible/filtrable aun en directorios/mapa/ficha, slugs `andina`/`amazonica` vs 027 `andes`/`amazonia` (sin FK), etiqueta `#f-barrio` "Barrio / Zona" vs nuevo campo "Zona"; **BUG-002 CERRADO el 2026-09-21** (backticks del comentario L1646 y doble escape `\u2605` L2473 de `api/pagina-destino.js` ahora en 0; R3 de TSK-144 resuelta) y **doble toast de XP CERRADO** (deduplicacion: la UI suprime el toast local si el servidor ya trae `xp_detalle`); via futura `media_compartidos` (backend NO emite por `?tipo=multimedia_mapa`; requeriria cambio para volver a mostrar video/audio de comunidad compartidos en el drawer); docstring obsoleto en `mapa-cultural.js` L31-32 (lo corrige otro agente); cache de assets compartidos sin bump de ?v=N (BUG-073); documentos Core de ~1.28 MB que encarecen el contexto; BUG-061 y BUG-065 ABIERTOS; QA visual y shape real de ?tipo=mapa sin validar contra Neon; **BUG-079 (el fix solo evita huerfanos NUEVOS):** los votos de fotos curadas ya huerfanos por REPLACE previos siguen en Neon (medicion + re-anclaje pendientes); sin via para vaciar la galeria desde el admin (el caso "sin fotos" preserva lo existente a proposito); dependencia de `id_neon` del frontend (clientes legacy -> fallback por url unica; sin url valida -> 400 anti-perdida).
...
1. **[HECHO EN WORKING TREE, FALTA COMMIT] `scripts/smoke_ref_info.js`** existe (untracked) con 26 checks **26/26 PASS** y `package.json` (M) lo encadena como PRIMER smoke de `npm test`; solo falta incluirlos en el commit del lote.
2. **Commit + push del lote Pantallas de entrada (TSK-156):** `git add api/usuarios.js api/interacciones.js registro.html index.html` + commit + push (Vercel despliega al push; sin migraciones; sin cache-bust, `vercel.json` sirve todo `/(.*)\.js` con `no-store`).
3. **[QA runtime post-deploy]** con sesion real y codigo de referido real: el overlay aparece 1 sola vez (`ec_welcome_visto`) y no compite con el banner de bono-referido (z-index 10000 < 10002); el banner del registro muestra el NOMBRE del anfitrion; el reclamo post-alta acredita el premio elegido del catalogo; un codigo invalido muestra REFERIDO_INVALIDO y "Continuar sin invitacion" permite seguir.
...
- **TSK-156 SIN commit (el gate SÍ existe):** `scripts/smoke_ref_info.js` (26/26 PASS, untracked) y `package.json` (encadenamiento a `npm test`) NO estan commiteados; el riesgo real es que el commit del lote los OLVIDE y la red quede solo en local. Ejecutar `node scripts/smoke_ref_info.js` antes del deploy y commitear junto con el lote.
