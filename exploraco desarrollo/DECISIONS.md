...
### Nota de implementacion (2026-09-21) -- Fases 2, 3 y 4

**Estado del ADR: IMPLEMENTADO Y VERIFICADO (2026-09-21).** El contrato vigente sigue siendo el de la ENMIENDA 1; esta nota solo registra que el diseno se llevo a codigo, SQL y UI, y donde vive cada pieza (verificado contra archivo real, ADR-006).

- **Fase 2 (BD):** `db/migrations/031_gamificacion_v6_nivel_scaling.sql` (24495 bytes) creada, COMMITEADA (`c875675`) y **APLICADA en Neon** (11 sentencias OK, 2026-09-21). Contiene `usuarios.nivel_max smallint NOT NULL DEFAULT 1` con siembra unica desde la tabla VIEJA (ningun usuario pierde insignia), `gamificacion_config` (seed `cap_progresion=5.0`/`cap_global=10.0`/`m_nivel_max=3.0`), `xp_ledger` (13 columnas, multiplicadores `numeric(10,6)`, CHECK `cap_aplicado IN ('ninguno','progresion','global','accion')`, 2 indices) y el repricing ABSOLUTO de los 17 consumibles en `consumibles.precio_xp` con guard `information_schema` para las columnas de la 027 (no aplicada). Aditiva, idempotente (ADR-008) y ASCII-safe (ADR-002).
- **Fase 3 (backend), COMMITEADA en `9efbfc7` (HEAD):** `api/interacciones.js` **v25** (motor con `obtenerMultiplicadorNivel`/`leerConfigGamificacion`/`calcularXpFinal` con doble cap secuencial 5.0/10.0/`calcularXpAcreditado`/`armarXpDetalle`/`registrarXpLedger`/`esLiderDestino`/`completitudSpotAtributos`; catalogo unico `XP_BASES` con 19 claves; **29 call-sites** de `registrarXpLedger` y **20 call-sites** de `calcularXpAcreditado`; rama nueva `?tipo=spot_atributos`); `api/usuarios.js` **v19** (20 umbrales techo 42000 + campo `mult`, `nivel_visible = GREATEST(nivel derivado, nivel_max)`, costos de eleccion 800/500/500); `api/admin.js` **v4** (rama nueva `GET ?recurso=salud_red`, espejo `NIVEL_DERIVADO_SQL`, degradacion 42P01). **8/8 endpoints INTACTO** (ADR-001/ADR-010).
- **Fase 4 (frontend, COMMITEADA en `d803ce7` y PUSHEADA a `origin/main`):** los **8 espejos** de umbrales sincronizados (`api/usuarios.js` fuente, `api/interacciones.js`, `admin.html`, `usuario-session.js`, `index.html`, `comunidad.html`, `niveles-data.js`, `api/admin.js`) + los 20 titulos; errata corregida en la fuente (titulo 11 `Estrat\u00e9ga` -> `Estratega Comunitario`); toast con desglose de XP y deduplicacion (incluida la ficha dinamica y los conectores legacy); barra de progreso (`progresoNivel`); UI informativa de cupo (`mostrarEstadoCupo`); pestana admin "Salud de la Red" + leaderboard; textos de repricing en `mi-perfil.html`. **DESPLEGADA (2026-09-21): `main` = `origin/main` = `d803ce7` (Vercel despliega al push); cache-bust NO requerido porque `vercel.json` sirve todo `/(.*)\.js` con `Cache-Control: no-store`.**
- **Verificacion:** `npm test` **VERDE (exit 0)** con `scripts/smoke_gamificacion_v6.js` (30 checks) y `scripts/smoke_niveles_espejos.js` (10/10) nuevos, mas `smoke_test_gamificacion_v4.js` 95/95, `smoke_038_casas_clases.js` 78/78, `smoke_021_xp_decimal_rankings.js` 45/45, `smoke_036_compartir.js` 55/55, `smoke_visita_geocerca.js` 15/15 y `smoke_niveles_data.js` 31/31. Nuevos scripts npm `test`/`smoke:gamificacion`/`smoke:espejos`.
...
## ADR-055: Mercado de Emprendedores -- 3 mercados por Casa, habilidad global `Emprendedor` sobre `usuarios.mercado_puntos` (metrica de progreso) y compra atomica por CTEs

**ID:** ADR-055
**Fecha:** 2026-09-23
**Estado:** **IMPLEMENTADO EN WORKING TREE (2026-09-23);** verificado contra archivo real (ADR-006), `npm test` VERDE (exit 0) y `scripts/smoke_mercado.js` 38/38. **Pendiente operativo BLOQUEANTE:** aplicar `db/migrations/034_mercado_emprendedores.sql` en Neon -> deploy del backend (v28/v21/v5) -> frontend (sin cache-bust: `vercel.json` sirve todo `/(.*)\.js` con `Cache-Control: no-store`).
**Autor:** architect (AI-DOS); decision de producto del operador.

**Alcance:** `db/migrations/034_mercado_emprendedores.sql` (NUEVA, aditiva/idempotente/ASCII-safe), `api/interacciones.js` (**v28**: GET `?tipo=mercado_config|mercado_ofertas|mercado_mi` + POST `?tipo=mercado_publicar|mercado_comprar|mercado_cancelar|mercado_producir`), `api/usuarios.js` (**v21**: expone `mercado_puntos`/`mercado_nodo` en el perfil), `api/admin.js` (**v5**: `?recurso=mercado`), `mercado.js` (NUEVO, `window.Mercado`), `comunidad.html` (tab Mercado), `mi-perfil.html` (card Emprendedor), `admin.html` (pantalla Mercado), `scripts/smoke_mercado.js` (NUEVO, encadenado a `npm test`). Sin endpoints nuevos (**8/8 intacto**, ADR-001/ADR-010).

### Problema
El sistema de Casas (ADR-028/ADR-038/ADR-041) ya exponia `arancel_inter_casa`/`fee_mercado_interno` pero NO los cobraba, y no existia un espacio de comercio de consumibles entre usuarios. Se necesita un mercado con normas propias por Casa sin introducir una segunda moneda (ADR-018 fija `usuarios.xp_total` como moneda unica) y sin conceder capacidades funcionales ni multiplicadores de XP a traves de nodos de progresion (regla dura del arbol de RAMAS, `api/interacciones.js` ~L916-921).

### Opciones evaluadas
1. **Moneda propia del mercado** (segundo saldo gastable/transferible). Descartada: viola la moneda unica `xp_total` de ADR-018 y habilita arbitraje/farming entre saldos.
2. **Reusar `xp_total` como moneda del mercado.** Descartada: mezcla la economia de XP con la de consumo y altera el de-nivel de ADR-018.
3. **`usuarios.mercado_puntos` como METRICA DE PROGRESO** derivada de las ventas (no gastable, no transferible) + precios del mercado pagados en XP (ELEGIDA).

### Decision
1. **3 mercados INDEPENDIENTES, uno por Casa** (`condor|jaguar|delfin`, ADR-028), aislados por la columna `casa` de `mercado_ofertas`/`mercado_ventas`. Normas propias por Casa en `mercado_config` (PK `casa`): `impuesto_base_pct`, `arancel_inter_casa_pct`, `slots_base`, `permite_cross_casa`, `permite_produccion`, `precio_min`, `precio_max`, `duracion_oferta_horas`, `activo`. Semilla idempotente (`ON CONFLICT (casa) DO NOTHING`): **condor 2%/25%, jaguar 5%/10%, delfin 0%/5%**.
2. **`usuarios.mercado_puntos numeric(12,2)` es METRICA DE PROGRESO, NO moneda:** no es gastable ni transferible; se deriva de las ventas (`neto/10`). **NO viola la moneda unica `usuarios.xp_total` de ADR-018.** Los precios del mercado se pagan en XP.
3. **Habilidad GLOBAL "Emprendedor"** (aplica en las 3 Casas): catalogo en codigo `MERCADO_NODOS` de 5 nodos con tiers `[0,100,250,450,700]` (misma escala del Arbol de Clases, Regla de No-Duplicidad). Efectos **ACOTADOS a la economia del mercado**: `slots_extra`, `reduccion_impuesto` y `produce`. `impuesto_efectivo = max(0, impuesto_base_pct - reduccion del nodo)` con **piso 0** (Delfin puede quedar en 0%). `slots_totales = slots_base + slots_extra`.
4. **Gate doble (aplicado):** el nodo efectivo de efectos es `min(nodo por puntos, nodo por nivel de jugador)`, con `nivel_jugador` por nodo `[2,5,10,20,30]`. **Estado de enforcement en el archivo real (ADR-006):** el gate doble SI se aplica. Helpers `nodoMercadoPorNivel(nivelJugador)` y `calcularMercadoEfectivo(puntos, nivelJugador)` en `api/interacciones.js` v28 (junto a `calcularMercado`/`nodoMercado`); el nodo EFECTIVO alimenta `mercado_mi` (GET owner-only con sesion firmada; expone `nodo_por_puntos` y `nodo_por_nivel`), `mercado_publicar`, `mercado_comprar` (nodo del vendedor) y `mercado_producir`. Espejo `calcularMercadoLocal(puntos, nivelJugador)` con `MERCADO_NIVELES = [2,5,10,20,30]` en `api/usuarios.js` v21, aplicado en `conNivel` y en el perfil publico.
5. **Cross-Casa permitido** con `arancel_inter_casa_pct` (sujeto a `permite_cross_casa` de la Casa del mercado). **Autocompra prohibida** (403 `AUTOCOMPRA_PROHIBIDA`).
6. **Compra atomica en UNA sentencia con CTEs** (NO `FOR UPDATE`): el driver neon HTTP no mantiene transacciones interactivas, por lo que un unico statement `WITH ok_oferta/ok_xp/debit/dec/cred/venta` descuenta XP del comprador, acredita el neto al vendedor + `mercado_puntos` (`neto/10`), hace merge JSONB del inventario (ADR-003), baja `cantidad_restante` y registra `mercado_ventas`. **`23514` -> 409 `OFERTA_NO_DISPONIBLE`** (y `23503`/`22P02` -> 400 en publicar).
7. **Anti-farming:** tope de **20 compras por comprador / 24h** (429 `LIMITE_COMPRAS_24H`), autocompra prohibida (403) y cross-Casa no permitido (403 `CROSS_CASA_NO_PERMITIDO`).
8. **Produccion de consumibles** `tipo_canje='producir'` (`prod_artesania` 40 XP, `prod_cafe` 60 XP, `prod_souvenir` 30 XP): consume XP (`precio_xp_base x cantidad`) y deposita unidades en el inventario; exige nodo con `produce` (nodos 3-5) y `permite_produccion`.
9. **RECONCILIACION con la regla dura del arbol de RAMAS (`api/interacciones.js` ~L916-921):** `MERCADO_NODOS` es un **catalogo SEPARADO** de `RAMAS`. La regla "los nodos de RAMAS no conceden capacidades funcionales ni multiplicadores de XP" aplica al arbol de 16 ramas y **NO** a este catalogo. Los efectos del mercado estan **acotados a la economia del mercado** (slots de oferta, reduccion del impuesto de la Casa, produccion de consumibles) y **NO crean multiplicadores de XP ni conceden privilegios globales**. La justificacion queda registrada aqui (docs-keeper), como anticipa el comentario del propio catalogo.
10. **Admin (`api/admin.js` v5, `?recurso=mercado`):** `mercado_config_lista`, `mercado_config_editar` (valida rangos server-side), `mercado_ofertas_lista` (filtros `casa`/`estado`) y `mercado_ofertas_moderar` (moderar SOLO cancela; no borra, ADR-003). Degradacion 42P01/42703 con mensaje que cita la 034.

### Justificacion
Es el unico modelo que habilita comercio entre usuarios sin una segunda moneda y sin romper la regla dura de no otorgar capacidades/multiplicadores por nodos de progresion: `mercado_puntos` mide progreso de mercado (no se gasta), los efectos de los nodos mueren dentro de la economia del mercado, y el aislamiento por Casa es de DATOS (columna `casa`), no de RLS. La compra en un unico statement con CTEs resuelve la atomicidad impuesta por el driver HTTP sin `FOR UPDATE` interactivo. Todo entra por ramas `?tipo=`/`?recurso=` existentes (8/8, ADR-001/ADR-010).

### Consecuencias
Positivas: tres mercados aislados con normas propias y semilla idempotente; moneda unica preservada (ADR-018); compra atomica verificable; ledger de ventas append-only (`mercado_ventas`) con desglose completo (subtotal/impuesto/arancel/neto); produccion de consumibles acotada por nodo; **gate doble de nivel APLICADO en la misma entrega** (nodo efectivo = `min(puntos, nivel)`; helpers `nodoMercadoPorNivel`/`calcularMercadoEfectivo` y espejo `calcularMercadoLocal`).
Negativas / **deuda aceptada:**
- **`mercado_puntos` no tiene ledger por evento** (se deriva de las ventas; a diferencia de `xp_ledger`, no es auditable evento a evento).
- **Migracion 034 obligatoria antes del deploy:** sin ella las ramas responden 503 `SCHEMA_NOT_MIGRATED` (42P01/42703), sin romper.
- **Reconciliacion 027/034:** la 034 declara `precio_xp_base`/`precio_xp_actual`/`tipo_canje` y las auto-provisiona de forma idempotente en su **seccion 4-bis** (NO-OP si la 027 ya corrio; la 027 fue aplicada en Neon el 2026-09-23). El aislamiento por Casa es de DATOS, no de RLS (la 034 no crea policies ni funciones).
