...
- **Fase 4 (frontend, working tree SIN commitear):** los **8 espejos** de umbrales sincronizados (`api/usuarios.js` fuente, `api/interacciones.js`, `admin.html`, `usuario-session.js`, `index.html`, `comunidad.html`, `niveles-data.js`, `api/admin.js`) + los 20 titulos; errata corregida en la fuente (titulo 11 `Estrat\u00e9ga` -> `Estratega Comunitario`); toast con desglose de XP y deduplicacion (incluida la ficha dinamica y los conectores legacy); barra de progreso (`progresoNivel`); UI informativa de cupo (`mostrarEstadoCupo`); pestana admin "Salud de la Red" + leaderboard; textos de repricing en `mi-perfil.html`. **Pendiente: commit/push + Vercel con cache-bust.**
