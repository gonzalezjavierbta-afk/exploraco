...
- **Pendiente operativo (BLOQUEANTE para produccion):** **deploy del frontend** (commit/push + Vercel) con cache-bust de los assets tocados para que los 8 espejos, el toast con desglose, la barra de progreso, la UI de cupo y la pestana "Salud de la Red" lleguen a produccion. La migracion 031 YA esta en Neon y el backend ya esta commiteado; el frontend sigue en working tree. Orden: 031 (hecho) -> backend v25/v19/v4 (`9efbfc7`) -> frontend.
