...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      if (params.tipo === 'mapa' && params.usuario_id) {
        const rows = await sql`
          ...
          WHERE i.usuario_id = ${params.usuario_id}
            AND i.tipo = 'guardado'
      ...
      if (params.tipo === 'resenas' && params.destino_id) {
        const rows = await sql`
          ...
          WHERE i.destino_id = ${params.destino_id}
            AND i.tipo = 'resena'
          ORDER BY i.creado_en DESC
          LIMIT 20
        `;
      ...
      if (params.tipo === 'is_guardado' && params.usuario_id && params.destino_id) {
        const rows = await sql`
          ...
          WHERE usuario_id = ${params.usuario_id}
            AND destino_id = ${params.destino_id}
            AND tipo = 'guardado'
        `;
    ...
    if (event.httpMethod === 'POST') {
      ...
      if (tipo === 'resena') {
        if (!b.rating) {
          return { statusCode: 400, headers, body: JSON.stringify({ ok: false, error: 'Rating requerido (1-5)' }) };
        }
        const rows = await sql`
          ...
          ON CONFLICT (usuario_id, destino_id, tipo) DO UPDATE SET
