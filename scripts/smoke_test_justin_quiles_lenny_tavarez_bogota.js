const seed = require('./seed-justin-quiles-lenny-tavarez-bogota.js');
...
check('Edicion y sede en tarjetas', html.includes('Superarte Tour') && inc(seed.TAGS.sede));
check('Lineup con Quiles y Tav\u00e1rez', html.includes('id="lineup"') && inc('Justin Quiles') && inc('Lenny Tav\u00e1rez') && (html.match(/Escenario principal/g) || []).length >= 2);
check('Agenda con puertas y show', html.includes('Apertura de puertas') && inc('Inicio del show'));
check('Tipos de entrada AGOTADO', html.includes('id="tipos-entrada"') && html.includes('Agotado') && html.includes('$180.000'));
