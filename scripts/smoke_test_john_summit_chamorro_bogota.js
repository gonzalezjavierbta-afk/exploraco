const seed = require('./seed-john-summit-chamorro-bogota.js');
...
check('Fecha y sede con fecha formateada', html.includes('id="evento-fechas"') && html.includes('10 de Septiembre de 2026'));
check('Edicion y sede en tarjetas', inc('John Summit - Show ben\u00e9fico en Colombia') && inc(seed.TAGS.sede));
check('Lineup con John Summit', html.includes('id="lineup"') && inc('John Summit') && inc('Natalia Par\u00eds') && (html.match(/Escenario principal/g) || []).length >= 4);
check('Agenda con puertas y show', html.includes('Apertura de puertas') && inc('Inicio del show electr\u00f3nico'));
check('Tipos de entrada Disponible', html.includes('id="tipos-entrada"') && html.includes('Disponible'));
check('Que llevar / prohibido', html.includes('id="que-llevar"') && inc(seed.TAGS.que_llevar[0]) && inc(seed.TAGS.prohibido[0]));
check('Mapa con coordenadas Chamorro City Hall', html.includes('4.6515'));
