const seed = require('./seed-sabor-bogota.js');
...
const d = Object.assign({}, seed.BASE, { fotos: [{ url: seed.HERO }], tags: seed.TAGS, rating: 4.8, total_resenas: 1, creado_en: new Date(), actualizado_en: new Date() });
...
check('Fecha y sede con fecha formateada', html.includes('id="evento-fechas"') && html.includes('2 de Septiembre de 2026'));
check('Edicion y sede en tarjetas', html.includes('Edicion 2026') && inc('Plaza de Mercado La Concordia'));
check('Lineup con Guardianes de Semilla', html.includes('id="lineup"') && html.includes('Guardianes de Semilla') && html.includes('Cocineros de La Concordia'));
check('Agenda con experiencia gastronomica', html.includes('Sabor Bogota') && html.includes('soberania alimentaria'));
...
check('Mapa con coordenadas', html.includes('4.5969'));
