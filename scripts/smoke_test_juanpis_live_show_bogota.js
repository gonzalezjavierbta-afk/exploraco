const seed = require('./seed-juanpis-live-show-bogota.js');
...
check('Fecha y sede con fecha formateada', html.includes('id="evento-fechas"') && html.includes('29 de Agosto de 2026'));
check('Edicion y sede en tarjetas', html.includes('Si Nos Organizamos Cabemos Todos') && inc(seed.TAGS.sede));
check('Lineup con 13 artistas', html.includes('id="lineup"') && inc('Feid') && inc('Carlos Vives') && inc('ChocQuibTown') && inc('Monsieur Perin\u00e9') && (html.match(/Escenario principal/g) || []).length >= 12);
check('Agenda con puertas y show', html.includes('Apertura de puertas') && inc('Inicio del show ben\u00e9fico'));
check('Tipos de entrada AGOTADO', html.includes('id="tipos-entrada"') && (html.match(/tip-red/g) || []).length >= 4 && html.includes('Agotado') && html.includes('$330.000'));
check('Que llevar / prohibido', html.includes('id="que-llevar"') && inc(seed.TAGS.que_llevar[0]) && inc(seed.TAGS.prohibido[0]));
check('Mapa con coordenadas Movistar Arena', html.includes('4.6652'));
