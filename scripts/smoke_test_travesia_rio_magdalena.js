const seed = require('./seed-travesia-rio-magdalena.js');
...
const d = Object.assign({}, seed.BASE, { fotos: [{ url: seed.HERO }], tags: seed.TAGS, rating: 4.7, total_resenas: 1, creado_en: new Date(), actualizado_en: new Date() });
...
check('Fecha y sede con fechas formateadas', html.includes('id="evento-fechas"') && html.includes('2 de Septiembre de 2026') && html.includes('6 de Septiembre de 2026'));
check('Edicion y sede en tarjetas', html.includes('Edicion 2026') && inc('Brazo de Loba'));
check('Lineup con joropo', html.includes('id="lineup"') && html.includes('joropo') && html.includes('currulao'));
check('Agenda con campamentos', html.includes('campamento') || html.includes('campamentos'));
check('Tipos de entrada', html.includes('id="tipos-entrada"') && html.includes('Paquete de travesia completa') && html.includes('Cupos limitados'));
check('Que llevar / prohibido', html.includes('id="que-llevar"') && inc(seed.TAGS.que_llevar[0]) && inc(seed.TAGS.prohibido[0]));
check('Mapa con coordenadas', html.includes('id="mapa"') && html.includes('9,-74.3'));
