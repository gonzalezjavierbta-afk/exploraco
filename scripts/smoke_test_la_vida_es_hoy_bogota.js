const seed = require('./seed-la-vida-es-hoy-bogota.js');
...
check('Edicion y sede en tarjetas', html.includes('Edicion Bogota 2026') && inc(seed.TAGS.sede));
check('Lineup con conductores', html.includes('id="lineup"') && html.includes('Camilo Cifuentes') && html.includes('Miguel Buitrago'));
check('Agenda con show completo', html.includes('show completo') && html.includes('7:00 pm'));
check('Tipos de entrada con boletaenlinea', html.includes('id="tipos-entrada"') && html.includes('boletaenlinea.co'));
check('Que llevar', html.includes('id="que-llevar"') && inc(seed.TAGS.que_llevar[2]));
check('Mapa con coordenadas EAN Legacy', html.includes('4.6628'));
