const seed = require('./seed-festival-teatro-libre-bogota.js');
...
check('Fecha y sede con fechas formateadas', html.includes('id="evento-fechas"') && html.includes('31 de Agosto de 2026') && html.includes('7 de Septiembre de 2026'));
check('Edicion y sede en tarjetas', html.includes('Tercera edicion') && inc('Teatro Libre Sede Centro'));
check('Lineup con obras clasicas', html.includes('id="lineup"') && html.includes('Tartufo') && html.includes('Coriolano') && html.includes('El Inspector'));
check('Agenda con funcion diaria', html.includes('Enfermo Imaginario') && html.includes('Pequeno Teatro de Medellin'));
check('Tipos de entrada boleteria', html.includes('id="tipos-entrada"') && html.includes('Boleteria por funcion') && html.includes('Disponible'));
check('Que llevar / prohibido', html.includes('id="que-llevar"') && inc(seed.TAGS.que_llevar[0]) && inc(seed.TAGS.prohibido[0]));
check('Mapa con coordenadas', html.includes('4.6006'));
