const seed = require('./seed-stray-kids-bogota.js');
...
check('Fecha y sede con fecha formateada', html.includes('id="evento-fechas"') && html.includes('9 de Septiembre de 2026'));
check('Edicion y sede en tarjetas', html.includes('Stray City 2026 - Gira mundial') && inc(seed.TAGS.sede));
check('Lineup con los 8 integrantes', html.includes('id="lineup"') && inc('Felix') && inc('Hyunjin') && (html.match(/Escenario principal/g) || []).length >= 8);
...
check('Mapa con coordenadas Vive Claro', html.includes('4.647'));
