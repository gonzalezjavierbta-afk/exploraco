const seed = require('./seed-parranda-vallenata-barranquilla.js');
...
check('Fecha y sede con fecha formateada', html.includes('id="evento-fechas"') && html.includes('5 de Septiembre de 2026'));
check('Edicion y sede en tarjetas', html.includes('Mes de Amor y Amistad - Apertura en TRUQ') && inc(seed.TAGS.sede));
check('Lineup con Heredero y Rey Vallenato', html.includes('id="lineup"') && inc('Samuel Morales') && inc('Jaime Luis Campillo') && (html.match(/Escenario principal/g) || []).length >= 3);
check('Agenda con puertas y parranda', html.includes('Apertura de puertas') && inc('Inicio de la parranda vallenata'));
...
check('Mapa con coordenadas Barranquilla', html.includes('11.0065'));
