const seed = require('./seed-queentaesencia-homenaje-queen-medellin.js');
...
check('Fecha y sede con fecha formateada', html.includes('id="evento-fechas"') && html.includes('11 de Septiembre de 2026'));
check('Edicion y sede en tarjetas', html.includes('Noche Queen - Tributo Queentaesencia') && inc(seed.TAGS.sede));
check('Lineup con Queentaesencia', html.includes('id="lineup"') && inc('Queentaesencia') && (html.match(/Escenario principal/g) || []).length >= 1);
check('Agenda con puertas y show', html.includes('Apertura de puertas') && inc('Inicio del show tributo a Queen'));
check('Tipos de entrada con cover', html.includes('id="tipos-entrada"') && html.includes('$50.000') && html.includes('Disponible'));
check('Que llevar / prohibido', html.includes('id="que-llevar"') && inc(seed.TAGS.que_llevar[0]) && inc(seed.TAGS.prohibido[0]));
check('Mapa con coordenadas Trilogia Bar', html.includes('6.22564'));
