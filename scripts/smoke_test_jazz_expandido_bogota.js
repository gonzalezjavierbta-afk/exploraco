const seed = require('./seed-jazz-expandido-bogota.js');
...
check('Edicion y sede en tarjetas', html.includes('Temporada Jazz Expandido 2026') && inc(seed.TAGS.sede));
check('Lineup con artistas internacionales', html.includes('id="lineup"') && inc('Tiken Jah Fakoly') && inc('Rembrandt Trio') && inc('Luca Ciarla'));
check('Agenda con fechas clave', html.includes('Apertura con Emergentes Jazz') && inc('homenaje a Louis Armstrong'));
check('Tipos de entrada con Agotado', html.includes('id="tipos-entrada"') && html.includes('Disponible') && html.includes('Agotado'));
check('Que llevar / prohibido', html.includes('id="que-llevar"') && inc(seed.TAGS.que_llevar[0]) && inc(seed.TAGS.prohibido[0]));
check('Mapa con coordenadas del CNA', html.includes('4.5967'));
