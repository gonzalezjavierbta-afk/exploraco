// scripts/load-monserrate-guia-api.js
// Carga el post de blog monserrate-guia-completa.html a traves de la
// API de admin de produccion (/api/admin-destinos), enviando el MISMO
// payload que genera admin.html (_placeToAPI/_buildTagsObj) para la
// categoria blog.
//
// Uso:
//   node scripts/load-monserrate-guia-api.js [URL] [TOKEN]
...
const { SLUG, HERO, PHOTOS, BASE, TAGS, FAQS, FOTOS_GALERIA } = require('./seed-monserrate-guia.js');
...
(async function main() {
  ...
  console.log('OK - blog ' + json.data.slug + ' (' + json.data.id + ') status=' + json.data.status);
  console.log('Palabras cuerpo: ~' + BASE.descripcion.trim().split(/\s+/).filter(Boolean).length);
  console.log('Temas: ' + TAGS.temas.join(', ') + ' | Video: ' + TAGS.video_url);
  console.log('Fotos galeria: ' + FOTOS_GALERIA.length + ' | FAQs: ' + FAQS.length);
